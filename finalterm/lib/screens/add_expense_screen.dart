...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  void initState() {
    ...
    if (widget.expenseDoc != null) {
      ...
      _splitEqually = data['isSplit'] ?? true;
    } else {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (widget.expenseDoc != null) {
          ...
          final oldIsSplit = oldData['isSplit'] as bool;
          final oldSplitAmount = oldIsSplit ? (oldAmount / totalMembers) : 0.0;
        ...
        final newSplitAmount = _splitEqually ? (amount / totalMembers) : 0.0;
        for (var member in widget.members) {
          final memberId = member['id'];
          final currentBalance = (balances[memberId] as num? ?? 0.0).toDouble();
              ...
              Row(
                children: [
                  Checkbox(
                    ...
                    onChanged: (value) {
                      if (value != null) {
                        setState(() => _splitEqually = value);
                      }
                    },
                    checkColor: Colors.white,
                    activeColor: Colors.green,
                  ),
                  const Text('Split this expense equally', style: TextStyle(color: Colors.white)),
                ],
              ),
            ],
          ),
